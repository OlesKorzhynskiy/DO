...
namespace Lab1
{
    public abstract class AbstractSimplexTable
    {
        ...
        public bool IsAllVariablesInt()
        {
            if (Math.Round(FunctionResult) != Convert.ToInt32(FunctionResult))
                return false;

            for (int i = 0; i < FreeValues.Length; i++)
                if (Math.Round(FreeValues[i], 1) != Convert.ToInt32(FreeValues[i]))
                    return false;
            return true;
        }
